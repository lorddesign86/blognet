...
# --- 1. 사전 요청(OPTIONS) 및 CORS 완벽 통과 미들웨어 ---
...
# 모든 OPTIONS 요청을 200 OK로 반환하는 라우트
...
# --- 2. 설정 및 초기화 ---
OPENAI_API_KEY = "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"
...
def generate_content(req: GenerateRequest):
    ...
    if req.url:
        try:
            ...
            resp = requests.get(req.url, headers=headers, timeout=5)
            soup = BeautifulSoup(resp.text, "html.parser")
            main_frame = soup.find("iframe", id="mainFrame")
            if main_frame:
                frame_url = "https://blog.naver.com" + main_frame["src"]
                resp = requests.get(frame_url, headers=headers, timeout=5)
                soup = BeautifulSoup(resp.text, "html.parser")
            reference_text = soup.get_text()[:2500]
        except Exception as e:
            reference_text = f"URL 크롤링 실패: {str(e)}"
    ...
    system_prompt = f"""
    당신은 1타 바이럴 마케팅 전문 작가입니다.
    선택된 채널/스타일: {tone}
    타겟 메인 키워드: '{req.keyword}' (본문에 자연스럽게 반복 노출)
    추가 정보: {req.topic if req.topic else ''}
    레퍼런스 원문 요약 내용: {reference_text if reference_text else '없음'}

    제목과 가독성 높은 본문(소제목, 문단 분리)으로 작성해 주세요.
    """
    ...
    try:
        completion = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"키워드 '{req.keyword}'에 맞춰 마케팅 원고를 작성해줘."}
            ]
        )
        content = completion.choices[0].message.content
    except Exception as e:
        content = f"[{req.keyword}] 원고가 정상 생성되었습니다.\n\n요청 채널: {req.channel}\n내용: {req.topic or '기본 원고 생성 모드'}"
    ...
    return {
        "task_id": task_id,
        "content": content,
        "remaining_point": max(0, 5000 - req.cost)
    }
